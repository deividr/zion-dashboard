...
export function OrderTicketPreview({ data }: OrderTicketPreviewProps) {
    ...
    return (
        <Card className="mx-auto max-w-[320px] font-mono text-xs">
            <CardHeader className="pb-2">
                <CardTitle className="text-center text-lg font-bold">LaBuonapasta</CardTitle>
                <CardDescription className="text-center">Pedido #{data.orderNumber}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-2">
                ...
                {data.customerName && (
                    <div className="border-b border-t border-dashed py-2">
                        <p>
                ...
                <div className="space-y-1">
                    {data.items.map((item, index) => (
                        <div key={index} className="grid grid-cols-12 gap-1">
                            ...
                            <span className="col-span-4 text-right">{formatCurrency(item.price)}</span>
                        </div>
                    ))}
                </div>
                ...
                {data.paymentMethod && (
                    <p className="text-center text-muted-foreground">Pagamento: {data.paymentMethod}</p>
                )}

                <p className="pt-2 text-center text-muted-foreground">Obrigado pela preferência!</p>
            </CardContent>
        </Card>
    );
}
